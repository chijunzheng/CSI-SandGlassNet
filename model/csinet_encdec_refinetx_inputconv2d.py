...
class CsiNet_EncDecTX_inputConv(nn.Module):
    def __init__(self, M=512, Nc=32, Nt=32):
        super(CsiNet_EncDecTX_inputConv, self).__init__()
        ...
        self.input_conv = nn.Conv2d(2, 2, kernel_size=4, padding='same')
    ...
    def forward(self, x):
        # input
        x = F.leaky_relu(self.input_bn(self.input_conv(x)), negative_slope=0.3)
